...
/// Every mapping of a direction whose two sides no longer hold the same bytes.
///
/// What the audit verb reports. The comparison is the transfer's own, run
/// without the write, so a finding here and a transfer's `Updated` cannot
/// disagree about whether two sides agree.
///
/// # Errors
///
/// The run-level refusals [`import`] raises, and whatever reading either side
/// failed with.
pub fn diverged(workspace: &Workspace) -> Result<Vec<Divergence>> {
    let bridge = workspace.bridge()?;
    if bridge.mappings.is_empty() {
        return Ok(Vec::new());
    }
    let Some(flake) = bridge.clan_flake.clone() else {
        return Ok(Vec::new());
    };
    let clan = Clan::new(flake);
    // A consumer with mappings and no clan installed is not a drift finding: it
    // is a machine that cannot answer the question, and `check` reporting
    // divergence it could not have observed would be a false finding.
    if clan.probe().is_err() {
        return Ok(Vec::new());
    }

    let mut found = Vec::new();
    for mapping in &bridge.mappings {
        let held = held_by_safix(workspace, mapping)?;
        let theirs = clan.read(
            &mapping.id,
            &mapping.clan.machine,
            &mapping.clan.generator,
            &mapping.clan.file,
        )?;
        let agree = match (&held, &theirs) {
            (Some(ours), Reading::Present(theirs)) => ours.equals(theirs),
            (None, Reading::AbsentAtSource) => true,
            _ => false,
        };
        if !agree {
            found.push(Divergence {
                ...
                safix_holds: held.is_some(),
                clan_holds: matches!(theirs, Reading::Present(_)),
            });
        }
    }
    Ok(found)
...
/// One mapping whose two sides disagree.
///
/// Which side is newer is not among these fields, and that is deliberate: neither
/// store records when a value was set in a form the other can be compared
/// against, so a claim about which moved last would be a guess.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Divergence {
    /// The mapping's declared name.
    pub mapping: String,
    /// Which way it is declared to move.
    pub direction: Direction,
    /// Whether safix's side holds a value at all.
    pub safix_holds: bool,
    /// Whether clan's side holds a value at all.
    pub clan_holds: bool,
}

/// The mapping's endpoints, for a report line that has to name them.
#[must_use]
pub fn endpoints(mapping: &Mapping) -> String {
    format!(
        "{} {} <-> flake.safix.users.{}.{}",
        mapping.clan.machine,
        Clan::var_id(&mapping.clan.generator, &mapping.clan.file),
