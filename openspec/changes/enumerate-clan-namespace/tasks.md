...
- [ ] 2.5 Update `clan.rs`'s module doc comment from "the three contracts" (read, write, machine discovery — `sync-clan-vars-two-way`'s own addition) to name the fourth, and record there that enumeration never sends a machine's vars to standard input and never reads a secret var's value
...
- [ ] 3.3 Compute the claimed set from that same narrowed set, per design.md's D3: a `(machine, id)` pair for every per-machine-placement mapping, using its declared `mapping.clan.machine`; an `id` alone, matched machine-insensitively, for every shared-placement mapping — `mapping.direction` never enters either comparison
...
- [ ] 5.8 Verify: `cargo test -p safix` passes, covering all of 5.1-5.7
