...
Every subcommand but `generate` SHALL behave identically under `--entry` as against a flake; `generate` is the documented exception.
`install` is identical under either because it evaluates no nix at all.
The requirement SHALL state the exception rather than a count of the subcommands or of the attributes, so a verb or a declaration attribute added in a later change does not leave a number behind that nothing checks.
...
- **WHEN** an entry file's declarations include at least one generator, one bridge mapping, one keepassxc mapping, and one 1password mapping
- **THEN** the JSON `--entry` evaluation emits for `generatorPlan`, `bridge`, `keepassxc`, and `onepassword` deserializes without an unknown-field error against the same `#[serde(deny_unknown_fields)]` structs the flake path deserializes against
