...
let
  ...
  # The three storage roots the registry carries, or today's spellings when a
  # caller passes a bare fixture registry. The header's two worked examples are
  # committed into every consumer's `.sops.yaml`, so after a rename they would
  # otherwise document a layout the repository does not have — a false
  # statement in the one file a reviewer reads to learn who can open what.
  storageOf = registry: registry.storage or resolve.defaultStorage;
  ...
  header = storage: ''
    ...
    ${storage.encrypted}/shared/<a>,<b>/, named for its members in sorted order so the
    ...
    Anchoring survives every edit to the generator, in two independent ways:
      ...
      2. Every `path_regex` is anchored with `^`. sops matches `path_regex`
         ...
         `nested/${storage.encrypted}/users/alice/x.yaml` would match a rule written for
         `${storage.encrypted}/users/alice/`. `[^/]*` rather than `.*` keeps each rule to
  ...
  render = registry: renderPlan (storageOf registry) (plan registry);
