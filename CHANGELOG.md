...
- Nine checks over the enrollment target, driven against a card surface that records what it was handed: `safix-enroll`, `-provisioned`, `-backup`, `-refusals`, `-one-attempt`, `-proof`, `-proof-isolation`, `-hook` and `-custody`.
