...
Rerunning a plan whose journal still exists resumes it: recorded outputs are re-verified — identity, bytes, and for ciphertext a fresh decryption through the target identities — and kept; the rest are published. A journal written for another plan refuses naming both plans; a recorded output whose identity or bytes changed refuses by name and is neither removed nor overwritten.
`safix migrate --abandon <plan.json>` discards an interrupted run: exactly the outputs the journal records and the staging directories it names, then the journal. It refuses with no journal, with another plan's journal, or over a changed output. Sources are retained on every path.
An interrupting signal between steps rolls the migration back the way any other recoverable error does and exits 130 or 143. The verb gains the integration suite it never had: a plan fixture over real age and sops ciphertext, the rollback scenario, resumption, abandonment, the refusals and a signal drill.

**The picker edits its query and shows its work.** `safix view` keeps a caret: typed characters insert where it stands, Backspace and Delete take the character on either side, Left/Right move it, Home/End and ^A/^E jump to the ends in every encoding terminals send. Ctrl+Left and Ctrl+Right scroll the columns; the plain arrows no longer do. An escape sequence the picker does not bind is ignored instead of cancelling.
The column titles moved under the alphabetically first row, and a full-width rule carrying the pane's title separates the list from the decrypted value, with one blank line above the rule and one below the value. A terminal too short for the pane keeps the rows and their titles.
The characters each term matched are bold and underlined inside the cell they matched, and the titles of the columns the query searched are emphasised: one title for a field term, all of them for a bare term, none for an exclusion or an empty query. Lines are cut by visible width, so no emphasis is split.

**Values have deadlines.** `flake.safix.rotation.<policy>.every` declares an interval once — `<n>h`, `<n>d` or `<n>w` — and an entry opts in with `rotation = "<policy>"`; an entry naming an undeclared policy fails evaluation by name. Every resolved placement carries the policy and its interval in seconds.
`safix list` and the picker gain a `ROTATES` column: `12d 04:12:09`, `due`, or `-`. The picker recomputes it every frame, so it ticks while the picker is open, and a due row is red. A governed value with no timestamp record is due at once.
`safix check` reports every value past its deadline, naming `safix rotate` for a generator-backed entry and `safix set` for a typed one, decrypting nothing. `safix rotate <user> <name>` re-mints one value now with the cascade its dependents require; `safix rotate --due [--yes] [<user>]` re-mints everything overdue in dependency order with cascades merged, lists the due typed entries it cannot mint, and exits zero. `safix rotation set|unset <user> <name> [<policy>]` edits the policy in the entry's own declaration, parsed before staging and committed.
`safix.rotation.{enable,repository,onCalendar,environment}` on a home-manager profile installs a persistent systemd user timer running `safix rotate --due --yes` in the named repository. Off by default; the identity must decrypt without a prompt; the timer commits and pushes nothing; machines receive rotated values on their next rebuild.

**The runtime reads a real fleet again, and the suite is green end to end.** `model::Placement` accepts the `neededForUsers`, `restartUnits` and `reloadUnits` fields the resolver has emitted on every placement since the deployment work, so `safix --entry examples/plain-nix/entry.nix list` resolves instead of refusing with `safix::nix_schema_mismatch`. Nine checks the previous change left red are repaired: the installer refuses a non-symlink directory at its symlink path by name instead of failing on `EINVAL`; the user-scope identity preflight states its ordering remedy and its limit again; `identity backup pgp` stages its verification keyring under a 23-byte name, because GnuPG binds its agent socket inside the home when no `/run/user/<uid>` exists and `sun_path` is 108 bytes — a home longer than 83 bytes could not start an agent; the vault fixture configures a git author; and the stale manifest snapshot and structural literals follow the fields the modules now emit.

**The README is a quickstart and the manual moved to `docs/`.** `README.md` walks one plain NixOS flake end to end — a person, a host, a service, a generator, a rotation policy, a template, a home profile — with every nix block lifted from `examples/quickstart/`, which one check evaluates as a real host and home profile and another diffs byte for byte against the README's fences. `docs/` carries the tutorial, the guides (including unattended hosts, inspectable recipients, what to do after removing access, rotation, migration recovery and flake-parts), the concepts, and the reference for every verb, option, refusal code, environment variable and the migration plan schema, each documented once.
