...
Thirteen of safix's fourteen verbs behave identically under `--entry`; `generate` additionally needs `--nixpkgs <flake-ref>` or `SAFIX_NIXPKGS`, because the generator sandbox resolves its tools through a flake regardless of how the declarations themselves were reached.
